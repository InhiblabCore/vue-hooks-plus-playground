...
const defaultVueUrl = import.meta.env.PROD
	? `${location.origin}/vue-hooks-plus-playground/play/vue.runtime.esm-browser.js` // to be copied on build
...
async function hashId(filename: string) {
	const nid = nanoid(20)
	const msgUint8 = new TextEncoder().encode(`${filename}+${nid}`) // encode as (utf-8) Uint8Array
	const hashArray = Array.from(new Uint8Array(msgUint8)) // convert buffer to byte array
